...
public class Main {
    ...
    private static int countEqualProcess(List<Integer> list, int processCount, int baseLine){
    	if(sumArr(list)==(list.size()*baseLine))
    		return processCount;    	
    	equal(list, baseLine);
    	return countEqualProcess(list, processCount = processCount+1, baseLine);
    }
    private static void equal(List<Integer> list, int baseLine){
    	for(int j=0; j<list.size(); j++){
	    	int diff = (list.get(j) - baseLine);
	    	int mVal = 0;
	    	if(diff>=5)
	    		mVal = 5;	    	
	    	if(diff<5 && diff >=2)
	    	...
	    	if(diff<2 && diff >=1)
	    		mVal = 1;	    	
	    	list.set(j, list.get(j)-mVal);
    	}
    	
    	System.out.println(list);    	
    }
    private static int sumArr(List<Integer> list){
    	int sum = 0;
